...
def initiate_flags():
    global IMAGE_SIZE, n_records

    tfrecords_filename = './TFRecords_files/Splits_%s/full_train.tfrecords'%(split_size)
...
def train():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        num_steps_per_epoch = (
            float(n_records)) / (FLAGS.batch_size * FLAGS.num_gpus)
        ...
        dataset = dataset.apply(tf.contrib.data.shuffle_and_repeat(buffer_size=n_records))
        ...
        for step in xrange(FLAGS.max_steps):
            ...
            if step % 10 == 0:
                ...
                format_str = ('\nSplit %s-full: %s: step: %d; epoch: %d; loss = %.2f,\n \
                                last_tpr = %.2f, last_fpr = %.2f, last_comp = %.2f, last_mixed_tp_fp = %.2f \n \
                                (%.1f examples/sec; %.3f sec/batch)\n')
                print (format_str % (split_size, datetime.now(), step, epoch, loss_value, \
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    max_steps = pickle.load( open( n_steps_file, "rb" ) )
